...
```
```{r}
gIntersection(stream, macleish_layers[["boundary"]], byid=TRUE)
```
...
```{r}
gIntersection(polyblue, polyblack@proj4string, unaryUnion_if_byid_false=FALSE)
```
...
In white radius blue stream and black boundary. Not in buildings, research, challenge courses. In all forest except the two forest types of the other two campsites
